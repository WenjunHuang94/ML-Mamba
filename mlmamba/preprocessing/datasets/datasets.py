...
PyTorch Dataset Definitions for MLMamba models; supports processing for both the `align` and `finetune` stages, with
...
class FinetuneDataset(Dataset[Dict[str, torch.Tensor]]):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        prompt_builder, input_ids, labels = self.prompt_builder_fn(model_family="mlmamba"), [], []
        for turn_idx, turn in enumerate(conversation):
            ...
            turn_labels = (  # When dealing with loss functions in dialogue systems, we do not want to consider the loss caused by prompt messages such as "User:<msg>", but only focus on the corresponding reply part
