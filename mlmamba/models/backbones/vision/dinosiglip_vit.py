...
from mlmamba.models.backbones.vision.base_vision import ImageTransform, LetterboxPad, VisionBackbone, unpack_tuple
...
class DinoSigLIPViTBackbone(VisionBackbone):
    def __init__(self, vision_backbone_id: str, image_resize_strategy: str, default_image_size: int = 224) -> None:
        ...
        self.dino_featurizer.eval()  # Not training

        self.siglip_featurizer: VisionTransformer = timm.create_model( # Create a Vision Transformer model and set the pretrained=True parameter to load the pre trained weights
        ...
        self.dino_featurizer.forward = unpack_tuple(  # As long as there are features in the middle layer of the model, that's good
            partial(self.dino_featurizer.get_intermediate_layers, n={len(self.dino_featurizer.blocks) - 2})
        )
        self.siglip_featurizer.forward = unpack_tuple(  # Taking the middle layer can extract better image features
        ...
        self.dino_data_cfg["input_size"] = (3, self.default_image_size, self.default_image_size)  # Modify the input size of the model
        ...
        default_dino_transform = timm.data.create_transform(**self.dino_data_cfg, is_training=False)  # Get transform
        ...
        if self.image_resize_strategy == "resize-naive":
            ...
            dino_transform = Compose(
                [
                    ...
                    *default_dino_transform.transforms[1:],  # Exclude the first transformation operation, which is default-dinou_transform.transforms [0], which adjusts the image size to the target size
                ]
            )
