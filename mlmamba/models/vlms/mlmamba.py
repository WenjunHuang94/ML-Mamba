...
PyTorch Module defining a MLMambaVLM, our general interface for defining the various different VLMs in our work.
...
from mlmamba.models.backbones.llm import LLMBackbone, MambaLLMBackbone
...
from mlmamba.models.vlms.base_vlm import VLM
from mlmamba.overwatch import initialize_overwatch
from mlmamba.util.nn_utils import FusedMLPProjector, LinearProjector, MLPProjector, FusedLDPProjector
from mlmamba.models.mamba.modeling_mamba import GenerationMixin as MambaGenerationMixin
...
class MLMambaVLM(VLM):
    def __init__(
        ...
        super().__init__(  # Assign values to visic_backbone and llm_backbone through the constructor of VLM base class
            "mlmamba",
            model_id,  # 'mlmamba+3b'
            vision_backbone,
            llm_backbone,
        ...
        self.mlp = nn.Sequential(  # The total dim length of image features ->llm_dim
                nn.Linear(vision_backbone.embed_dim, llm_backbone.embed_dim, bias=True),
                nn.GELU(),
            )
        ...
        if arch_specifier == "linear":
            self.projector = LinearProjector(llm_backbone.embed_dim, llm_backbone.embed_dim)
        elif arch_specifier.endswith("vision-to-llm"):
            self.projector = FusedMLPProjector(vision_backbone.embed_dim, llm_backbone.embed_dim)
        elif arch_specifier.endswith("fused-gelu-mlp"):  # arch_specifier = 'no-align+fused-gelu-mlp'
            #self.projector = FusedMLPProjector(vision_backbone.embed_dim, llm_backbone.embed_dim)
            self.projector = FusedMLPProjector(llm_backbone.embed_dim, llm_backbone.embed_dim)
        elif arch_specifier.endswith("gelu-mlp"):
            self.projector = MLPProjector(llm_backbone.embed_dim, llm_backbone.embed_dim)
        elif arch_specifier.endswith("fused-ldpnet"):
            self.projector = FusedLDPProjector(llm_backbone.embed_dim, llm_backbone.embed_dim)
        else:
            raise ValueError(f"MLMambaVLM with `{arch_specifier = }` is not supported!")
    ...
    def mamba_generate(self, *args, **kwargs):
        ...
        # There is a difference between this and the original MLMAMBA. Please pay attention to the modifications
    ...
    @classmethod
    def from_pretrained(
        cls,  # <class 'mlmamba.models.vlms.mlmamba.MLMambaVLM'>
        pretrained_checkpoint: Path,
        model_id: str,  # 'mlmamba+3b'
        vision_backbone: VisionBackbone,
        ...
        """Initialize a MLMambaVLM from a pretrained checkpoint, freezing all weights, tailored for inference."""
        vlm = cls(
            model_id,
            vision_backbone,
            llm_backbone,
        ...
        ), "MLMambaVLM `from_pretrained` expects checkpoint with keys for `projector` AND `llm_backbone`!"
    ...
    def load_from_checkpoint(self, stage: str, run_dir: Path, pretrained_checkpoint: Optional[Path] = None) -> None:
        ...
        # (1) The original only loaded pre training parameters for the Vision module and Mamba module, and MLP did not load pre training
        # (2) Change to loading the latest self trained checkpoint
        ...
        if self.arch_specifier.startswith("no-align"):  # arch_specifier = 'no-align+fused-gelu-mlp'
            overwatch.info(
                f"MLMambaVLM with `{self.arch_specifier = }` does not require pretrained weights!", ctx_level=1
            )
            return
    ...
    def get_fsdp_wrapping_policy(self) -> Callable:
        ...
        # Get MLMamba Wrapping Policy =>> just a module wrapping policy around `self.projector`
        mlmamba_fsdp_wrapping_policy = partial(
            _module_wrap_policy,
    ...
    def forward(
        ...
        with torch.set_grad_enabled(self.vision_backbone_requires_grad):
            if isinstance(pixel_values, dict):
                ...
                return self.llm_backbone(  # If there are no pictures, generate answers directly
                input_ids=input_ids,
                attention_mask=None,
                position_ids=None,
                past_key_values=past_key_values,
        ...
        if labels is not None:  # Enter during training
            projected_patch_labels = torch.full(
                (projected_patch_embeddings.shape[0], projected_patch_embeddings.shape[1]),
                IGNORE_INDEX,  # Ignore this label and do not include the predicted part of the image in the training calculation loss
                dtype=labels.dtype,
        ...
        unimodal_indices = torch.tensor(  # Idx of unimodal data
    ...
    def generate(self, image: Image, prompt_text: str, **kwargs: str) -> str:
        ...
        # Ignore the original sentence and only decode the predicted content
