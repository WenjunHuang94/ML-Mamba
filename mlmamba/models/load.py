...
# === HF Hub Repository ===
HF_HUB_REPO = "anonymous405/ML-Mamba"
...
def load(
    ...
    if os.path.isdir(model_id_or_path):
        ...
        config_json = os.path.join(os.path.dirname(__file__), '..', 'conf', 'config.json')
        ...
        # checkpoint_pt = './latest-checkpoint.pt'

    # Load Model Config from `config.json`
    with open(config_json, "r") as f:
        model_cfg = json.load(f)["model"]

    model_cfg['llm_backbone_id'] = "mamba2-2.7b"  # Change to the mamba2 model you need
