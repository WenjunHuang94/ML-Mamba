...
def decode(
    ...
    def get_logits(input_ids, inference_params):
        ...
        if decoding:  # The second time after this
        ...
        if not use_cache or not decoding:
            logits = model(
                ...
                num_last_tokens=1,
                return_dict=True,
                pixel_values=pixel_values,  # An image transformed by two visual models
            ).logits.squeeze(dim=1)  # （B,l,vocab_size)->（B,vocab_size)  (1,1,50280) -> (1,50280)
        else:  # Entering here for the second time
            logits = model._decoding_cache.run(
                input_ids, position_ids, inference_params.seqlen_offset
            ).squeeze(dim=1)  # (1,1,50280) -> (1,50280)
    ...
    if max_new_tokens is not None: # The length will be covered here
...
class MambaModel(MambaPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs
        ...
        return BaseModelOutput(last_hidden_state=hidden_states)  # Here, hidden_states=(B, L, D) will be packaged as a list [(B, L, D)] and returned
...
class MambaForCausalLM(MambaPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **backbone_kwargs
        ...
        hidden_states = outputs[0]  # Only take the first element, which is also (B, L, D)!!!
        ...
        loss = None
        if labels is not None:  # Only entered during training
            # Shift so that tokens < n predict n
            shift_logits = logits[..., :-1, :].contiguous()
            shift_labels = labels[..., 1:].contiguous()
            # Flatten the tokens
            loss_fct = CrossEntropyLoss()  # In the CrossEntropyLoss function, the default value of ignore_index is -100, and its loss is not calculated
            ...
            loss = loss_fct(shift_logits, shift_labels)  # When training to calculate the loss, attention mask was not actually used and the value of -100 was ignored without calculating the loss
