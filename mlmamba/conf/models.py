...
class ModelConfig(ChoiceRegistry):
    ...
    enable_mixed_precision_training: bool = True            # Turning off the mixing precision will result in an error, let's turn it on
...
@dataclass
class MLMamba_3B(ModelConfig):
    model_id: str = "mlmamba+3b"
    arch_specifier: str = "no-align+fused-gelu-mlp"

    vision_backbone_id: str = "dinosiglip-vit-so-384px"
    llm_backbone_id: str = "mamba2-2.7b"  # Modify as needed
...
#   =>> Note :: Run with `--dataset.type "llava-lvis4v-lrv"`
@dataclass
class MLMamba_3B_One_Epoch(MLMamba_3B):
    model_id: str = "mlmamba-one-epoch+3b"
    finetune_epochs: int = 1


#   =>> Note :: Run with `--dataset.type "llava-lvis4v-lrv"`
@dataclass
class MLMamba_3B_Base(MLMamba_3B):
    model_id: str = "mlmamba-base+3b"
    llm_backbone_id: str = "mamba-2.8b-slimpj"
    
    
#   =>> Note :: Run with `--dataset.type "llava-lvis4v-lrv"`
@dataclass
class MLMamba_3B_Siglip(MLMamba_3B):
    model_id: str = "mlmamba-siglip+3b"
    vision_backbone_id: str = "siglip-vit-so400m-384px"
    arch_specifier: str = "no-align+gelu-mlp"
    
#   =>> Note :: Run with `--dataset.type "llava-lvis4v-lrv"`
@dataclass
class MLMamba_3B_LDPNET(MLMamba_3B):
    model_id: str = "mlmamba-ldpnet+3b"
...
class ModelRegistry(Enum):
    MLMAMBA_3B = MLMamba_3B
    MLMAMBA_ONE_EPOCH_3B = MLMamba_3B_One_Epoch
    MLMAMBA_BASE_3B = MLMamba_3B_Base
    MLMAMBA_SIGLIP_3B = MLMamba_3B_Siglip
    MLMAMBA_LDPNET_3B =  MLMamba_3B_LDPNET
