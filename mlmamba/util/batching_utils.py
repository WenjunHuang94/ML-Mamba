...
class SplitModalitySampler(Sampler):  # Used to group samples in the dataset according to modality (such as images and text) and length, so that each device (such as GPU) sees samples with similar sequence lengths during training
    def __init__(
        self,
        dataset: Dataset,
