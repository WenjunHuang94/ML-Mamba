...
class PretrainConfig:
    ...
    dataset: DatasetConfig = field(
        default_factory=DatasetConfig.get_choice_class(DatasetRegistry.LLAVA_V15.dataset_id)  # Replace with LVIS-Instruct-4V
    )
...
def pretrain(cfg: PretrainConfig) -> None:
    ...
    worker_init_fn = set_global_seed(cfg.seed, get_worker_init_fn=True)  # Use the same random number seed to ensure reproducibility
    ...
    vlm.load_from_checkpoint(cfg.stage, run_dir, pretrained_checkpoint=cfg.pretrained_checkpoint)  # The Vision module and Mamba module pre training parameters have been loaded, but MLP has not loaded pre training
    ...
    train_strategy = get_train_strategy(
        ...
        device_id=device_id,
        epochs=cfg.epochs,
        max_steps=cfg.max_steps,  # The value of self.max_steps is empty and needs to be changed!!!
    ...
    train_strategy.run_setup(run_dir=run_dir, n_train_examples=len(train_dataset))  # Set some parameters and strategies during the training process
